...
#fin1 : tissue type
#fin1 : peer factors depend on sample size
# the selection of PEER factors was optimized based on sample size. 
# sample size < 150, 15 PEER factors
# sample size ranging from 150 to 250, 30 PEER factors
# sample size > 250, 35 PEER factors 
...
SNP_file_name <- paste0("./GTEX/01.preprocess/6.snp_matrix/",fin1,".snpmatrix")
expression_file_name <- paste0("./GTEX/05.analysis_data/promoter_usage/",fin1)
covariates_file_name <- paste0("./GTEX/05.analysis_data/combine/",fin1,".covariates")
...
genepos = read.table(paste0("./GTEX/05.analysis_data/location/promoter/",fin1,".txt"),header=TRUE,stringsAsFactors=FALSE)
snpspos = read.table(paste0("./GTEX/05.analysis_data/location/snp/",fin1,".snp.loc"),header=TRUE,stringsAsFactors=FALSE)
output_file_name_cis = paste("./GTEX/06.puQTL/peer/",fin1,".txt",sep="");
output_file_name     = paste("./GTEX/06.puQTL/peer/",fin1,".txt",sep="")
...
verbose = TRUE
output_file_name.cis = paste("./GTEX/06.puQTL/peer/",fin1,".txt",sep="")
...
{	
  ...
  cvrt = t( qr.Q(q) );
  peer_num = 8 + as.numeric(fin2)
  q.ethnicity <- t( qr.Q( qr(t(cvrt[c(1,4:8),])) ) ) 		# QR decomposition of intercept and 3 PCs
  q.ethnicity.PEER <- t( qr.Q( qr(t(cvrt[c(1,4:peer_num),])) ) ) # QR decomp of intercept + 3PCs + 15 PEER
  q.ethnicity.PEER.gender <- t( qr.Q( qr(t(cvrt[c(1,4:peer_num,2),])) ) ) # QR decomp of interc + 3PCs + 15 PEER + gender
  q.PEER <- t( qr.Q( qr(t(cvrt[c(1,9:peer_num),])) ) )
...
################################# Gene expression processing ############################
{
  ...
  for( sl in 1:gene$nSlices() ) {
    ...
    projection <- lapply(1:nrow(slice), function(geneidx) {
      ...
      project_PEER <- slice[geneidx,] %*% QQ.PEER
    ...
    slice_PEER <- do.call(rbind, lapply(projection,'[[',6))
    rowsq7 = rowSums((slice-slice_PEER)^2)
  ...
  ss_stats <- data.frame( geneID = rownames(as.matrix(gene)),
                          ...
                          fraction_var_intercept_peer = 1 - SS_PEER/SS_total,
                          fraction_var_ethnicity = (SS_PEER-SS_ethnicity_PEER_residual)/SS_total,
